...
+(function() {
  ...
  var calculateScroll = function() {
    ...
    // 获取指定位置的元素
    var point = document.elementFromPoint($('.docs-content ').offset().left+50, 240);
    contentSectionItem = $(point).hasClass('component')?$(point):$(point).parents('.component:first');
    //if (contentSectionItem.is(old)) {
    //  return;
    //} else {
      old = contentSectionItem;
      ...
      if (!contentSectionItem.hasClass('informational')) {
        if (contentSectionItem.data("url")) {
          ...
          updateContent(contentSectionItem.find('.component-example').html());
        }
      }
